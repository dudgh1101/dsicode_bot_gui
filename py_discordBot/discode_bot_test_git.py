...
# print(os.getenv("TOKEN"))  # 디버그 출력 (필요시 활성화)
# pip install wheel
# pip install PyNaCl
#필수 설치 라이브러리
# ────────── 설정 ──────────
...
# ────────── GUI에서 받은 명령어 처리 ──────────
async def check_gui_command():
    """GUI에서 보낸 str_commend_line을 주기적으로 확인 및 처리"""
    ...
    while not client.is_closed():
        try:
            if str_commend_line and str_commend_line.strip():  # 명령어가 있으면
                command = str_commend_line.strip()
                print(f"🔔 GUI에서 명령어 수신: {command}")
                
                # 명령어 처리 (DM으로 전송한 것처럼 처리)
                ...
                if user:
                    ...
                    if command.startswith("!add"):
                        ...
                        if len(parts) >= 3:
                            _, time, msg = parts
                            user_scheduled_messages.setdefault(OWNER_ID, {}).setdefault(time, []).append(msg)
                            await user.send(f"✅ {time}에 메시지가 추가되었습니다: \"{msg}\"")
                        ...
                        if len(parts) >= 2:
                            _, time = parts
                            if time.lower() == "all":
                                user_scheduled_messages[OWNER_ID] = {}
                                await user.send("🗑 모든 예약 메시지가 삭제되었습니다.")
                            else:
                                if OWNER_ID in user_scheduled_messages and time in user_scheduled_messages[OWNER_ID]:
                                    del user_scheduled_messages[OWNER_ID][time]
                                    await user.send(f"🗑 {time} 예약 메시지를 삭제했습니다.")
                            print(f"✅ 명령어 처리 완료: {command}")
                    
                    elif command.startswith("!list"):
                        await send_remaining_schedule_dm(OWNER_ID, user)
                        print(f"✅ 명령어 처리 완료: {command}")
                    
                    elif command.startswith("!call_in"):
                        channel = client.get_channel(VOICE_CHANNEL_ID)
                        if channel and isinstance(channel, discord.VoiceChannel):
                            guild = channel.guild
                            if guild.voice_client is None:
                                await channel.connect()
                                await user.send(f"🎤 봇이 지정된 통화방 **{channel.name}** 에 입장했습니다!")
                                print(f"✅ 명령어 처리 완료: {command}")
                            else:
                                await user.send("⚠️ 이미 통화방에 연결되어 있습니다.")
                        else:
                            await user.send("❌ VOICE_CHANNEL_ID가 올바르지 않습니다.")
                    
                    elif command.startswith("!call_out"):
                        disconnected = False
                        ...
                        if disconnected:
                            await user.send("👋 봇이 통화방에서 퇴장했습니다.")
                            print(f"✅ 명령어 처리 완료: {command}")
                        else:
                            await user.send("❌ 봇이 통화방에 연결되어 있지 않습니다.")
                    
                    elif command.startswith("!commend_list"):
                        await user.send(
                            ...
                            ":eight_spoked_asterisk: !call_in: 봇을 음성 채널에 입장시킵니다.\n"
                            ":eight_spoked_asterisk: !call_out: 봇을 음성 채널에서 퇴장시킵니다.\n"
                            ":eight_spoked_asterisk: !commend_list: 이 명령어 목록을 표시합니다."
                        )
                        print(f"✅ 명령어 처리 완료: {command}")
...
async def on_ready():
    ...
    client.loop.create_task(check_gui_command())  # ✅ GUI 명령어 체크 추가
